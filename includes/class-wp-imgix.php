...
class WPImgIX
{
	...
	/**
	 * Register actions and filters, but only if basic ImgIX functions are available.
	 *
	...
	private function setup()
	{
		...
		add_filter('rest_request_before_callbacks', array($this, 'should_rest_image_downsize'), 999, 3);
		add_filter('rest_request_after_callbacks', array($this, 'cleanup_rest_image_downsize'), 999);
	...
	/**
	 * Identify images in post content, and if images are local (uploaded to the current site), pass through ImgIX.
	 *
	 * @param string $content
	 * @uses self::validate_image_url, apply_filters, imgix_url, esc_url
	 * @filter the_content
	 * @return string
	 */
	public static function filter_the_content($content)
	{
		...
		if (!empty($images)) {
			...
			foreach ($images[0] as $index => $tag) {
				...
				/**
				 * Allow specific images to be skipped by ImgIX.
				 *
				 * @since 2.0.3
				 *
				 * @param bool false Should ImgIX ignore this image. Default to false.
				 * @param string $src Image URL.
				 * @param string $tag Image Tag (Image HTML output).
				 */
				if (apply_filters('imgix_skip_image', false, $src, $tag))
					continue;
				...
				// Check if image URL should be used with ImgIX
				if (self::validate_image_url($src)) {
					...
					if (
						...
						(0 === strpos($src, $upload_dir['baseurl']) ||
							/**
							 * Filter whether an image using an attachment ID in its class has to be uploaded to the local site to go through ImgIX.
							 *
							 * @since 2.0.3
							 *
							 ...
							 * }
							 */
							apply_filters('imgix_image_is_local', false, compact('src', 'tag', 'images', 'index')))
					) {
						...
						if ($class_attachment_id) {
							...
							if (is_object($attachment) && !is_wp_error($attachment) && 'attachment' == $attachment->post_type) {
								...
								// use the attachment sources to determine the size. ImgIX modifies
					...
					// Build URL, first maybe removing WP's resized string so we pass the original image to ImgIX
					if (!$fullsize_url) {
						$src = self::strip_image_dimensions_maybe($src);
					}

					// Build array of ImgIX args and expose to filter before passing to ImgIX URL function
					$args = array();
					...
					/**
					 * Filter the array of ImgIX arguments added to an image when it goes through ImgIX.
					 * By default, only includes width and height values.
					 * @see https://developer.wordpress.com/docs/photon/api/
					 *
					 * @param array $args Array of ImgIX Arguments.
					 * @param array $args {
					 * 	 Array of image details.
					 *
					 * 	 @type $tag Image tag (Image HTML output).
					 ...
					 * }
					 */
					$args = apply_filters('imgix_post_image_args', $args, compact('tag', 'src', 'src_orig', 'width', 'height', 'attachment_id', 'size'));

					$imgix_url = imgix_url($src, $args);

					// Modify image tag if ImgIX function provides a URL
					// Ensure changes are only applied to the current image by copying and modifying the matched tag, then replacing the entire tag with our modified version.
					if ($src != $imgix_url) {
						$new_tag = $tag;

						// If present, replace the link href with a ImgIXed URL for the full-size image.
						if (!empty($images['link_url'][$index]) && self::validate_image_url($images['link_url'][$index]))
							$new_tag = preg_replace('#(href=["|\'])' . $images['link_url'][$index] . '(["|\'])#i', '\1' . imgix_url($images['link_url'][$index]) . '\2', $new_tag, 1);

						// Supplant the original source value with our ImgIX URL
						$imgix_url = esc_url($imgix_url);
						$new_tag = str_replace($src_orig, $imgix_url, $new_tag);

						// If Lazy Load is in use, pass placeholder image through ImgIX
						if (isset($placeholder_src) && self::validate_image_url($placeholder_src)) {
							$placeholder_src = imgix_url($placeholder_src);
						...
						if (apply_filters('imgix_remove_size_attributes', true)) {
					...
					$new_tag = preg_replace('#(href=["|\'])' . $images['link_url'][$index] . '(["|\'])#i', '\1' . imgix_url($images['link_url'][$index]) . '\2', $tag, 1);
	...
	/**
	 * Filter post thumbnail image retrieval, passing images through ImgIX
	 *
	 * @param string|bool $image
	 * @param int $attachment_id
	 * @param string|array $size
	 * @uses is_admin, apply_filters, wp_get_attachment_url, self::validate_image_url, this::image_sizes, imgix_url
	...
	{
		/**
		 * Provide plugins a way of enable use of ImgIX in the admin context.
		 *
		 * @since 0.9.2
		 *
		 * @param bool true Disable the use of ImgIX in the admin.
		 * @param array $args {
		 * 	 Array of image details.
		 *
		 ...
		 * }
		 */
		$disable_in_admin = is_admin() && apply_filters('imgix_disable_in_admin', true, compact('image', 'attachment_id', 'size'));

		/**
		 * Provide plugins a way of preventing ImgIX from being applied to images retrieved from WordPress Core.
		 *
		 * @since 0.9.2
		 *
		 * @param bool false Stop ImgIX from being applied to the image. Default to false.
		 * @param array $args {
		 * 	 Array of image details.
		 *
		 * 	 @type $image Image URL.
		 ...
		 * }
		 */
		$override_image_downsize = apply_filters('imgix_override_image_downsize', false, compact('image', 'attachment_id', 'size'));

		if ($disable_in_admin || $override_image_downsize) {
			return $image;
		}

		// Get the image URL and proceed with ImgIX-ification if successful
		...
		if ($image_url) {
			...
			// If an image is requested with a size known to WordPress, use that size's settings with ImgIX
			if ((is_string($size) || is_int($size)) && array_key_exists($size, self::image_sizes())) {
				...
				// Expose determined arguments to a filter before passing to ImgIX
				...
				// we don't want to lose this data. $image_args is used as the ImgIX URL param
				// args, so we want to keep the original image sizes args. For example, if the image
				// size is 300x300px, non-cropped, we want to pass `fit=300,300` to ImgIX, instead
				...
				// Check specified image dimensions and account for possible zero values; imgix fails to resize if a dimension is zero.
				if ((0 == $image_args['width'] || 0 == $image_args['height']) && $transform !== 'fit') {
					if (0 == $image_args['width'] && 0 < $image_args['height']) {
						$imgix_args['h'] = $image_args['height'];
					} elseif (0 == $image_args['height'] && 0 < $image_args['width']) {
						$imgix_args['w'] = $image_args['width'];
					}
				} else {
					...
					if ($is_intermediate && 'resize' === $transform && is_array($image_args['crop'])) {
						$imgix_args['crop'] = implode('', array_map(function ($v) {
							$map = [
								'top' => 'top',
								'center' => '',
								'bottom' => 'bottom',
				...
				/**
				 * Filter the ImgIX Arguments added to an image when going through ImgIX, when that image size is a string.
				 * Image size will be a string (e.g. "full", "medium") when it is known to WordPress.
				 *
				 * @param array $imgix_args Array of ImgIX arguments.
				 * @param array $args {
				 * 	 Array of image details.
				 *
				 ...
				 * }
				 */
				$imgix_args = apply_filters('imgix_image_downsize_string', $imgix_args, compact('image_args', 'image_url', 'attachment_id', 'size', 'transform'));

				// Generate ImgIX URL.
				$image = array(
					imgix_url($image_url, $imgix_args),
					$width,
					$height,
					$is_intermediate,
				);
				...
				/**
				 * Filter the ImgIX Arguments added to an image when going through ImgIX,
				 ...
				 * }
				 */
				$imgix_args = apply_filters('imgix_image_downsize_array', $imgix_args, compact('width', 'height', 'image_url', 'attachment_id'));
	...
	/**
	 * Filters an array of image `srcset` values, replacing each URL with its ImgIX equivalent.
	 *
	 * @since 3.8.0
	 * @param array $sources An array of image urls and widths.
	 * @uses self::validate_image_url, imgix_url
	 * @return array An array of ImgIX image urls and widths.
	 */
	public function filter_srcset_array($sources, $size_array, $image_src, $image_meta, $attachment_id)
	{
		...
		foreach ($sources as $i => $source) {
			 ...
			 * }
			 */
			$args = apply_filters('imgix_srcset_image_args', $args, compact('source', 'image_meta', 'width', 'height', 'attachment_id'));

			$sources[$i]['url'] = imgix_url($url, $args);
		}
	...
	/**
	 * Ensure image URL is valid for ImgIX.
	 *
	 * Though ImgIX functions address some of the URL issues, we should avoid unnecessary processing if we know early on that the image isn't supported.
	 *
	 * @param string $url
	 * @uses wp_parse_args
	 * @return bool
	...
	public static function validate_image_url($url)
	{
		...
		return apply_filters('imgix_validate_image_url', true, $url, $parsed_url);
	}

	/**
	 * Checks if the file exists before it passes the file to imgix
	 *
	 * @param string $src The image URL
	 * @return string
	 **/
	...
	{
		// Build URL, first removing WP's resized string so we pass the original image to ImgIX
	...
	/**
	 * Determine if image_downsize should utilize ImgIX via REST API.
	 *
	 * The WordPress Block Editor (Gutenberg) and other REST API consumers using the wp/v2/media endpoint, especially in the "edit"
	 * context is more akin to the is_admin usage of ImgIX (see filter_image_downsize). Since consumers are trying to edit content in posts,
	 * ImgIX should not fire as it will fire later on display. By aborting an attempt to change an image here, we
	 ...
	 * if determined we are using it in the edit context, we'll false out the `imgix_override_image_downsize` filter.
	 *
	...
	public function should_rest_image_downsize($response, $endpoint_data, $request)
	{
		...
		if (false !== strpos($route, 'wp/v2/media') && 'edit' === $request['context']) {
			...
			add_filter('imgix_override_image_downsize', array($this, '_override_image_downsize_in_rest_edit_context'), 999999);
		}
	...
	{
		remove_filter('imgix_override_image_downsize', array($this, '_override_image_downsize_in_rest_edit_context'), 999999);
		return $response;
	}

	/**
	 * Used internally by ::should_rest_image_downsize() to not imgix-ize
